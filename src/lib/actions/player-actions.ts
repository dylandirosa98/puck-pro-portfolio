...
export async function createPlayer(formData: FormData) {
  ...
  const row = {
    ...
    light_mode: formData.get("lightMode") === "true",
    custom_domain: (formData.get("customDomain") as string) || null,
    team_logo_url: (formData.get("teamLogoUrl") as string) || null,
    number_color: (formData.get("numberColor") as string) || null,
...
export async function deletePlayer(id: string) {
  ...
  const { data: player } = await supabase
    .from("players")
    .select("slug, highlights, skillsets, media, interests_media, training_videos, timeline")
    .eq("id", id)
    .single();
